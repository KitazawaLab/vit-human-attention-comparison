...
%% 
for gen_idx = 1:3
    gen = genres(gen_idx); % 1-3
    filename = strcat("subj2subj_vit_gbvs_CW2019_", string(gen), ".mat");
    disp(filename)
    load(filename)
    %% struct for save
    mds_res_all = struct;
    ...
    save_name = strcat("mds_results_CW2019_", string(gen), "_dim", string(embed_dim), ".mat");
    save(save_name, 'mds_res_all')
    clear mds_res_all
end
